import { Search, ClipboardCheck, Clock, CheckCircle, XCircle } from 'lucide-react';
...
function Claims() {
  ...
  return (
    <section className="min-h-screen bg-slate-50 p-6 md:p-10">
      ...
      <div>
        <h1 className="text-3xl font-bold text-slate-800">Claims</h1>

        <p className="mt-2 text-slate-500">Review and manage item ownership claims.</p>
      </div>
      ...
      <div className="mt-8 flex items-center rounded-xl bg-white px-4 py-3 shadow-sm">
        ...
        <input type="text" placeholder="Search claims..." className="ml-3 w-full outline-none" />
      </div>
          ...
          <tbody>
            {claims.map((claim) => (
              <tr key={claim.id} className="border-t">
                <td className="px-6 py-5 font-medium text-slate-800">{claim.item}</td>
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`rounded-full px-3 py-1 text-sm font-medium ${styles[status]}`}>{status}</span>
  );
}
