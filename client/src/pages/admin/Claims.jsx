...
function Claims() {
  ...
  const filteredClaims = claims.filter(
    (claim) =>
      String(claim.item).toLowerCase().includes(search.toLowerCase()) ||
      String(claim.user).toLowerCase().includes(search.toLowerCase()) ||
      claim.status.toLowerCase().includes(search.toLowerCase())
  );

  if (loading) {
    return <div className="p-10 text-slate-600">Loading claims...</div>;
  }
            ...
            {filteredClaims.length === 0 ? (
              ...
              filteredClaims.map((claim) => (
                <tr key={claim.id} className="border-t">
                  <td className="px-6 py-5 font-medium text-slate-800">Item #{claim.item}</td>

                  <td>User #{claim.user}</td>
